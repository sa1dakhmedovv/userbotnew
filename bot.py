    ...
    while True:
        ...
        try:
            ...
            for remaining in range(e.seconds, 0, -1):
                update_session(name, {"floodwait_remaining": remaining})
                await asyncio.sleep(1)
            update_session(name, {"floodwait_remaining": 0})
...
async def cmd_sessions(message: Message):
    ...
    for k, v in data.items():
        text += (f"\n✅ {k}\n"
                 ...
                 f"• Delay: {v['delay']}s\n"
                 f"• Status: {v['status']}\n")
        if v.get('floodwait_remaining', 0) > 0:
            text += f"• Floodwait: {v['floodwait_remaining']}s\n"
...
# ======== NEWSESSION ========
# Bu qism siz yuborgan kodingizda o'zgarishsiz qoladi.
# Men qirqib qo'yaman - faqat yuqoridagi PATCHlar yetarli.
